            ...
            <Container>
                <Row>
                    ...
                    {cards.items.map((item, index) => {
                        ...
                        console.log(item.image.replace("https://wintrau-001-site1.qtempurl.com", ""))
                        return (
                            <Col xs={6} md={4} key={index}>
                                {item.link ?
                                    <Link className={`${cards.alias}__card`} href={item.link}>
                                        <h3 className={`${cards.alias}__title`}>{item.title}<span>{item.text}</span></h3>
                                        <Image className={`${cards.alias}__image`} src={`${item.image.replace("https://wintrau-001-site1.qtempurl.com", "")}?mode=crop&width=500&height=500`} />
                                    </Link> :
